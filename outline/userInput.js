 ...
 */
    ...
    addNewObject: function addNewObject (e) {
      ...
      let preStat = insertTr.previousSibling.AJAR_statement
      if (!isInverse) { this.formUndetStat(insertTr, preStat.subject, preStat.predicate, reqTerm, preStat.why, false) } else { this.formUndetStat(insertTr, reqTerm, preStat.predicate, preStat.object, preStat.why, true) }
 ...
 */
            ...
            case 'BlankNode': // a request refill with text
              ...
              if (s.predicate.termType === 'Collection') { // case: add triple   ????????? Weird - tbl
                ...
                if (kb.any(undefined, selectedPredicate, textTerm)) {
                  ...
                  s3 = kb.anyStatementMatching(s.subject, rdf('type'), type, s.why)
                  // TODO: DEFINE ERROR CALLBACK
    ...
    */
            ...
            UI.store.updater.update([ ], [insertTr.AJAR_statement], function (uri, success, errorBody) {
              if (!success) {
                UI.log.error('userinput.js (pred selected): Fail trying to insert statement ' +
                  insertTr.AJAR_statement + ': ' + errorBody)
              }
            })
          } catch (e) {
    ...
    Refill: function Refill (e, selectedTd) {
      ...
      if (isPredicate) { // predicateTd
        ...
        if (selectedTd.parentNode.AJAR_statement.predicate.sameTerm(rdf('type'))) {
          let sparqlText = 'SELECT ?class WHERE{?class ' + rdf('type') + UI.ns.rdfs('Class') + '.}'
            ...
            switch (enterEvent.keyCode) {
              ...
              case 38:// up
                ...
                if (newText === '' && menu.lastHighlight.tagName === 'TR' &&
                                              !menu.lastHighlight.previousSibling) { setHighlightItem(menu.firstChild.firstChild) } else { setHighlightItem(menu.lastHighlight.previousSibling) }
                return "I'm a little Arrow Up"
              case 40:// down
                qp('handler: Arrow Down')
                movedArrow = true // hq
                if (menu.lastHighlight.tagName === 'INPUT') { setHighlightItem(menu.childNodes[1].firstChild) } else { setHighlightItem(menu.lastHighlight.nextSibling) }
                return "I'm a little Down Arrow"
              case 37:// left
              case 39:// right
                qp('handler: Arrow left, right')
                if (menu.lastHighlight.tagName === 'INPUT') {
                  if (enterEvent.keyCode === 37) { setHighlightItem(menu.lastHighlight.previousSibling) } else { setHighlightItem(menu.lastHighlight.nextSibling) }
                }
                return
              case 8:// backspace
    ...
    Keypress: function (e) {
      if (e.keyCode === 13) {
        if (outline.targetOf(e).tagName !== 'TEXTAREA') { this.clearInputAndSave() } else { // <TEXTAREA>
    ...
    // temporary key ctrl+s or q for swiching mode
      ...
      window.addEventListener('keypress', function (e) {
        if (e.ctrlKey && (e.charCode === 115 || e.charCode === 113)) UserInput.switchMode()
      }, false)
    ...
    whatSortOfEditCell: function whatSortOfEditCell (selectedTd) {
      ...
      if (kb.whether(predicateTerm, UI.ns.rdf('type'), UI.ns.owl('DatatypeProperty')) ||
           kb.whether(predicateTerm, UI.ns.rdfs('range'), UI.ns.rdfs('Literal')) ||
               predicateTerm.termType === 'Collection') { return 'DatatypeProperty-like' } else if (kb.whether(predicateTerm, rdf('type'), UI.ns.owl('ObjectProperty'))) { return 'ObjectProperty-like' } else { return 'no-idea' }
    },
    ...
    getStatementAbout: function getStatementAbout (something) {
      ...
      if (!trNode) throw new Error('No ancestor TR for the TD we clicked on:' + something)
      try {
    ...
    appendToPredicate: function appendToPredicate (predicateTd) {
      ...
      if (predicateTd.hasAttribute('rowspan')) { predicateTd.setAttribute('rowspan', parseInt(predicateTd.getAttribute('rowspan')) + 1) }

      if (!predicateTd.hasAttribute('rowspan')) predicateTd.setAttribute('rowspan', '2')

      if (!isEnd) { trIterator.parentNode.insertBefore(insertTr, trIterator) } else {
        var table = predicateTd.parentNode.parentNode
        if (table.className === 'defaultPane') { table.insertBefore(insertTr, table.lastChild) } else { table.appendChild(insertTr) }
      }
    ...
    generateRequest: function generateRequest (tipText, trNew, isPredicate, notShow) {
      var trNode
      if (!notShow) {
        if (trNew) { trNode = trNew } else { trNode = UI.utils.ancestor(this.lastModified, 'TR') }
      ...
      if (tipText.length < 10) { tp.add(reqTerm, UI.ns.link('message'), tp.literal(tipText)) } else { tp.add(reqTerm, UI.ns.link('message'), tp.literal(tipText)) }
    ...
    showMenu: function showMenu (e, menuType, inputQuery, extraInformation, order) {
      ...
      if (extraInformation) for (var x in extraInformation) console.log('\t extra ' + x + ': ' + extraInformation[x] + '\n')
      ...
      switch (menuType) {
        ...
        case 'TypeChoice':
          ...
          break
        default: throw new Error('userinput: unexpected mode')
      }
      ...
      switch (menuType) {
        ...
        case 'PredicateAutoComplete': // Prompt user  for possible relationships for new data
          ...
          for (let i = 0; i < entries.length && i < 10; i++) { // do not show more than 30 items
          ...
          for (let i = 0; i < juris.length; i++) {
            ...
            if (jTitleLowerCase.search(matchstr) !== -1) {
              ...
              matchedtitle.push(jtitle)
                        ...
                        // === Title, URI ==
              let tr = table.appendChild(myDocument.createElement('tr'))
              tr.setAttribute('about', 'journalTitle')
              let th = tr.appendChild(myDocument.createElement('th'))
        ...
        case 'LimitedPredicateChoice':
          ...
          for (let i = 0; i < choices.length; i++) { addMenuItem(choices[i]) }
          break
        default:
          ...
          switch (inputQuery.constructor.name) {
            case 'Array':
              for (let i = 0; i < inputQuery.length; i++) kb.query(inputQuery[i], addPredicateChoice(inputQuery[i]), nullFetcher)
              break
            case 'undefined':
    ...
    fillInRequest: function fillInRequest (type, selectedTd, inputTerm) {
      ...
      if (type === 'predicate') {
        ...
        if (!selectedTd.previousSibling || selectedTd.previousSibling.className !== 'undetermined') {
          let s
          if (!isInverse) { s = new UI.rdf.Statement(stat.subject, stat.predicate, inputTerm, stat.why) } else { s = new UI.rdf.Statement(inputTerm, stat.predicate, stat.object, stat.why) }
          ...
          try {
            UI.store.updater.update([], [s], function (uri, success, errorBody) {
              ...
              if (success) {
                ...
                if (!isInverse) { newStats = kb.statementsMatching(stat.subject, stat.predicate, inputTerm, stat.why) } else { newStats = kb.statementsMatching(inputTerm, stat.predicate, stat.object, stat.why) }
      ...
      if (doNext) { this.startFillInText(outline.getSelection()[0]) } else { return true } // can clearMenu
    },
