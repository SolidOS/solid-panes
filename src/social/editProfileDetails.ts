...
export function appendProfileLinks (
  ...
  for (const field of profileLinkFields) {
    ...
    for (const uri of uris) {
      ...
      let label = utils.label(field.predicate)
      if (uris.length > 1) {
...
export function createEditProfileDetailsButton (
  ...
  const openDialog = () => {
    ...
    const fields = new Map<ProfileLinkField['key'], HTMLTextAreaElement>()
    for (const field of profileLinkFields) {
      ...
      textarea.className = 'social-pane__dialog-input'
      textarea.rows = 2
      textarea.placeholder = field.placeholder
      textarea.value = initialValues[field.key]
      fieldWrapper.appendChild(textarea)

      fields.set(field.key, textarea)
    ...
    cancelButton.className = 'social-pane__dialog-button social-pane__dialog-button--secondary'
    cancelButton.textContent = 'Cancel'
    actions.appendChild(cancelButton)

    const saveButton = dom.createElement('button')
    saveButton.type = 'submit'
    saveButton.className = 'social-pane__dialog-button social-pane__dialog-button--primary'
