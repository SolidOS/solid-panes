...
module.exports = {
  icon: UI.icons.originalIconBase + 'social/social.gif',
  ...
  render: function (s, doc) {
    ...
    CustomEvents.prototype.raiseEvent = function (en, args, context) {
      ...
      if (!e[en]) {
        throw new Error('No event "' + en + '" registered.')
      }
    ...
    function newElement (tag, p) {
      ...
      if (!p) { socialpane.appendChild(x) } else { p.appendChild(x) }
      return x
    }
    ...
    var changeFriendButton = function () { // TODO update the text of the friend button
      ...
      if (myProfile) { ui.addFriend.className = 'hidden' } else { ui.addFriend.className = '' }
    }
